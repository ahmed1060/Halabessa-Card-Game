...
begin
  ...
  select profile.daily_reward into prior
  from halabessa.user_profiles as profile
  where profile.firebase_uid = p_uid
  for update;
  ...
  update halabessa.user_profiles as profile set
    coins = profile.coins + reward_coins,
    diamonds = profile.diamonds + reward_diamonds,
